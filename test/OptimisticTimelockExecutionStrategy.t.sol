...
// Most tests are similar to `TimelockExecutionStrategyTest.t.sol`.
abstract contract OptimisticTimelockExecutionStrategyTest is SpaceTest {
    error InvalidSpace();
...
contract OptimisticTimelockExecutionStrategyTestProxy is OptimisticTimelockExecutionStrategyTest {
    function setUp() public override {
        super.setUp();

        address[] memory spaces = new address[](1);
        spaces[0] = address(space);
        OptimisticTimelockExecutionStrategy masterExecutionStrategy = new OptimisticTimelockExecutionStrategy();

        timelockExecutionStrategy = OptimisticTimelockExecutionStrategy(
            payable(
                new ERC1967Proxy(
